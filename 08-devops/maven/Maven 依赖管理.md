...
`dependencyManagement` 更适合放在父 POM 中统一管理版本，它本身只负责“声明和约束版本”，并不会自动把依赖真正引入到子模块中。

典型好处有：

- 统一锁定多个子模块使用的依赖版本
- 子模块引用同一依赖时通常不必重复声明版本号
- 升级版本时只需要修改父 POM 一处
- 个别子模块如果需要特殊版本，仍然可以单独显式覆盖
...
子模块中如果需要真正使用该依赖，仍然要显式写出 `dependency`，只是可以省略版本：
